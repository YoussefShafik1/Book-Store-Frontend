...
export class CartService {
  ...
  private getAuthHeaders(): HttpHeaders {
    const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjoiNjdhZjQyOGM2OGZhZWRiZGQ1YTYxMjMyIn0sImlhdCI6MTczOTY1OTI0MCwiZXhwIjoxNzM5NjYyODQwfQ.IDjRhIe0kr1EA7n9ui0G6wr1H2jjlV2ekoLk72Y7Emo";
  ...
  getBookDetails(productId: string): Observable<BookDetails> {
    return this.http.get<BookDetails>(`${this.bookUrl}${productId}`, { headers: this.getAuthHeaders() });
  }

  getCartWithBookDetails(): Observable<{ items: CartItem[], totalAmount: number }> {
      ...
      switchMap(cart => {
        ...
        cart.items.forEach(item => {
          ...
          if (productId) {
            if (uniqueItems.has(productId)) {
              ...
              uniqueItems.set(productId, { productId, quantity: item.quantity });
            }
          }
        });
        ...
        const requests = Array.from(uniqueItems.keys()).map(productId =>
          this.getBookDetails(productId).pipe(
            map(bookDetails => {
              totalAmount += bookDetails.price * (uniqueItems.get(productId)?.quantity ?? 1);
              return {
                productId,
